...
export const RaceOverview: React.FC<Props> = ({
  data,
  selectedFields = [],
  theme,
}) => {
  ...
  return (
    <RaceStats
      items={items.filter(activity => selectedFields.includes(activity.title))}
    />
  )
}

const RaceOverviewWrapper: React.FC<Props> = ({ data, selectedFields }) => {
  return (
