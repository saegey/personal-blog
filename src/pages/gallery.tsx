...
const GalleryPage = ({ data }: PageProps<Data>) => {
  ...
  return <>
    <header className="max-w-3xl border-b border-line pb-10 sm:pb-14">
      ...
      <h1 className="mt-3 font-serif text-5xl leading-none tracking-[-0.04em] sm:text-7xl">Photographs from the road and elsewhere.</h1>
    ...
    <section className="py-10 sm:py-16">
      {galleries.length ? <div className="grid gap-8 sm:grid-cols-2 sm:gap-12">
        {galleries.map((gallery, index) => <Link key={gallery.slug} to={`/gallery/${gallery.slug}/`} className={`group block ${index % 2 ? 'sm:mt-20' : ''}`}>
          ...
          <div className="mt-4 border-t border-line pt-3"><p className="eyebrow">{gallery.theme}</p><h2 className="mt-2 font-serif text-3xl font-medium">{gallery.title}</h2><p className="mt-2 font-condensed text-sm uppercase tracking-label text-muted">{gallery.location} · {gallery.photos.length} photographs</p></div>
        </Link>)}
