...
headerImageCaption: 'An overhead view of part of the course which consisted mostly of farm land and well maintained gravel roads. You could call this "luxury" gravel. Photo: Adam Lapierre'
tags:
  - gravel
  - race journal
type: Race Journal
...
related:
  ...
  - 'Gran Fondo Winthrop'
teaser: This is a race you can't miss. Luxury gravel and a fast course with lots of climbing.
gpxFile: 'Gorge_Gravel_Grinder'
currentFtp: 273
statsFile: 'stats_gorge_gravel'
...
<Text>
  {console.log(props.data.mdx.statsData.fields.data['PowerAnalysis'])}
</Text>

<RaceOverview
  data={{
    elevationGain: props.data.mdx.statsData.fields.data['ElevationGain'],
    normalizedPower: props.data.mdx.statsData.fields.data['NormalizedPower'],
    heartAnalysis: {
      entire:
        props.data.mdx.statsData.fields.data['HeartAnalysis'][
          Math.max(
            ...Object.keys(
              props.data.mdx.statsData.fields.data['HeartAnalysis'],
            ).map(Number),
    ...
    powerAnalysis: {
      entire:
        props.data.mdx.statsData.fields.data['PowerAnalysis'][
          Math.max(
            ...Object.keys(
              props.data.mdx.statsData.fields.data['PowerAnalysis'],
            ).map(Number),
    ...
    elapsedTime: {
      seconds: props.data.mdx.statsData.fields.data['ElapsedTime'],
    },
    averageSpeed: props.data.mdx.statsData.fields.data['AverageSpeed'],
    stoppedTime: props.data.mdx.statsData.fields.data['StoppedTime'],
  }}
  selectedFields={[
    'Avg Power',
    'Avg Heart Rate',
...
<VisualOverview
  coordinates={props.data.mdx.statsData.fields.data['SimplifiedCoordinates']}
  elevationToAdd={500}
  elevationData={{
    downsampleRate: 1,
    data: props.data.mdx.statsData.fields.data['SimplifiedElevations'].map(
      (elevationFt, idx) => {
        const x = idx
        const y = elevationFt / 3.28084 // meters
        const dist =
          props.data.mdx.statsData.fields.data['SimplifiedDistances'][idx] // meters
        const prevDist =
          idx > 0
            ? props.data.mdx.statsData.fields.data['SimplifiedDistances'][
                idx - 1
        ...
        const prevY =
          idx > 0
            ? props.data.mdx.statsData.fields.data['SimplifiedElevations'][
                idx - 1
              ] / 3.28084
            : y
        const dx = Math.max(1, dist - prevDist) // avoid divide-by-zero
        const dy = y - prevY
        const grade = dx > 0 ? dy / dx : 0 // fractional grade
        return { distance: dist, y, grade, x }
      },
    ),
